...
export function createSettingsRouter(): Router {
  ...
  router.get('/', (_req, res) => {
    ...
    res.json({
      ...
      platformPrompt: getSetting('platform_prompt') ?? 'You are an AI agent. You have access to the working directory at {working_directory}.',
    })
  })

  // POST /api/settings — save platform config
  router.post('/', (req, res) => {
    const { platformPrompt } = req.body as {
